...
$DYN_productcat = esc_attr($product_categories);
$DYN_producttag = esc_attr($product_tags);
...
if($DYN_gallerycat || $DYN_productcat || $DYN_producttag) {
	...
	// WPEC Product Tags/Categories
	if($DYN_productcat) {
		$pcats = $DYN_productcat;
		$pcats = lTrim($pcats,',');	
	}	
...
if($DYN_productcat || $DYN_producttag) {
...
<?php if($DYN_gallerycat || $DYN_productcat || $DYN_producttag) { 
		if($DYN_gridfilter) { ?>
		...
		<ul class="splitter <?php echo "id-".esc_attr($id); ?>">
			<li><?php _e('Filter By: ', 'DynamiX' ); ?>
				<ul>
					...
					<li class="segment-<?php echo $catcount; ?>"><a href="#" data-value="<?php echo str_replace(" ","_",$strip_html).esc_attr($id); ?>"><?php echo $catname; ?></a></li>    
					<?php } 
					} elseif($DYN_productcat || $DYN_producttag) {
					
					$catcount=2;
					
					if($pcats && $tags) {
						$DYN_productcattag = 	$pcats .','. $tags;
					} elseif($pcats) {
						$DYN_productcattag = 	$pcats;
					} elseif($tags) {
						$DYN_productcattag = 	$tags;
					}
					
					$DYN_productcattag = explode(",", $DYN_productcattag);

					foreach ($DYN_productcattag as $catlist) { // Get category ID Array 
...
elseif($DYN_productcat || $DYN_producttag) :
	...
	if($wpsc_product_categories) {
		foreach ($wpsc_product_categories as $wpsc_product_category) {
			$categories .= str_replace(" ","_", $wpsc_product_category->name).esc_attr($id). ' ';
		}
	}
	...
	if($wpsc_product_tags) {
		foreach ($wpsc_product_tags as $wpsc_product_tag) {
			$categories .= str_replace(" ","_", $wpsc_product_tag->name).esc_attr($id). ' ';
		}
	}	
	$categories = preg_replace("/&#?[a-z0-9]+;/i","",$categories);
endif;
...
if($DYN_productcat || $DYN_producttag) :
$DYN_previewimgurl= wpsc_the_product_image(); // Product Image
$DYN_productprice= wpsc_the_product_price();
else :
...
if(esc_attr($excerpt)) {
 ...
 } elseif(esc_attr($imageeffect)=="shadow" || esc_attr($imageeffect)=="shadowblackwhite") {
 	if($DYN_stageplaypause=="enabled") {
...
<div id="cwz-accordion-<?php echo esc_attr($id); ?>" class="accordion-gallery-wrap <?php if($DYN_imageeffect=="shadow" || $DYN_imageeffect=="shadowblackwhite") { ?>shadow<?php } elseif($DYN_imageeffect=="reflection") { ?>reflection<?php } elseif($DYN_imageeffect=="shadowreflection") { ?>shadowreflection <?php } ?> <?php echo esc_attr($shadow); ?> <?php echo esc_attr($align); ?>"  style="width:<?php echo $DYN_gallerywidth; ?>px;">
...
jQuery().ready(function() {
	...
	<?php if($DYN_accordionautoplay="true") { ?>
	timeout: <?php echo $DYN_stagetimeout * 1000; ?>,
	<?php } ?>	
	duration: 800,
...
function blockquote_shortcode( $atts, $content = null ) {
   ...
   extract( shortcode_atts( array(
	...
	$insert_string = '<span class="quote right"><img src="'.get_bloginfo("template_url").'/images/blank.gif" alt="quote close" width="40" height="30" /></span>';	

	$newstring=substr_replace($content, $insert_string, $position, 0);
	

   return '<span class="' . esc_attr($type) .' '. esc_attr($align) .'"><span class="quote left"><img src="'.get_bloginfo("template_url").'/images/blank.gif" width="40" height="30" alt="quote open" /></span>' . $newstring . '</span>';
...
function imageeffect_shortcode( $atts, $content = null ) {
   extract( shortcode_atts( array(
	...
	$DYN_imgeffect = 'reflection'; 
	} elseif(esc_attr($type)=="shadowreflectlightbox" || esc_attr($type)=="shadowreflect") {
	$DYN_imgeffect = 'shadowreflection'; 
	...
	esc_attr($shadow) || 
	esc_attr($type)=="shadowblackwhite") { 
		$shadow =" shadow ".esc_attr($shadow); 
	} 
	
	if(esc_attr($link)!='') {
		$NV_link_start='<a href="'.esc_attr($link).'" title="'.esc_attr($alt).'" >';
		$NV_link_end='</a>';
	} else {
		$NV_link_start='';
		$NV_link_end='';
	...
	<div class="imagewrap <?php echo esc_attr($align); if($DYN_imgframe) echo ' '.$DYN_imgframe; ?>" style="height:<?php echo esc_attr($height); ?>px;width:<?php echo esc_attr($width); ?>px;">
        <div class="container <?php if($DYN_imgeffect) { echo $DYN_imgeffect; } echo $shadow; ?>">
            <div class="gridimg-wrap <?php echo $DYN_imgblackwhite; ?>">
				<div class="title-wrap" style="width:<?php echo $DYN_imgwidth; ?>px;">	

				<?php if($NV_lightbox && !$NV_link_start) { ?>
	...
	<div class="imagewrap <?php echo esc_attr($align); ?>" style="height:<?php echo $DYN_imgheight; ?>px;width:<?php echo $DYN_imgwidth; ?>px;">
		...
		<div class="container videotype <?php echo $DYN_videoshadow ?>">   
			<div class="gridimg-wrap">
				...
				<iframe  width="<?php echo $DYN_imgwidth; ?>" height="<?php echo $DYN_imgheight; ?>" src="http://www.youtube.com/embed/<?php echo $vidid.$splitter; ?>autoplay=<?php echo $DYN_videoautoplay ?>&amp;wmode=opaque&amp;title=" frameborder="0" allowfullscreen></iframe>
...
function tabs_shortcode( $atts, $content = null, $code ) {
   extract( shortcode_atts( array(
	...
	if( esc_attr($id)=="1") {
		return '<ul class="tabhead-wrap"><li class="tab-li"><h4 class="tabhead"><a href="#tabs-'. esc_attr($id).'">'. $content .'</a></h4></li>';
	} else {
		return '<li class="tab-li"><h4 class="tabhead"><a href="#tabs-'. esc_attr($id).'">'. $content .'</a></h4></li>';
	}
	} elseif($code=="tabhead_last") {
		return '<li class="tab-li"><h4 class="tabhead"><a href="#tabs-'. esc_attr($id).'">'. $content .'</a></h4></li></ul>';
	} elseif($code=="tab") {	
...
function accordion_shortcode( $atts, $content = null, $code ) {
   extract( shortcode_atts( array(
	...
	if($code=="accordion") { ?>
		<div class="accordion <?php if(esc_attr($collapse=='yes')) {  ?>collapse<?php } ?>"><?php echo do_shortcode($content); ?></div>
	...
	<?php } elseif($code=="panel") { ?>
		<div class="section <?php echo esc_attr($color) .' '. esc_attr($class); ?>"><h3 class="accordionhead"><a href="#"><?php echo html_entity_decode(esc_attr($title)); ?></a></h3><div class="sectioncontent"><?php echo do_shortcode($content); ?></div></div>
	<?php } ?>
...
function nv_recent_posts_shortcode($atts){
	...
	if(!$DYN_previewimgurl) { // check what image to use, custom, featured, image within post. 
		$post_image_id = get_post_thumbnail_id($post_to_use->ID);
			...
			$DYN_previewimgurl=catch_image(); // Check for images within post 
		}
	}	
	
	$image=$DYN_previewimgurl;
	
	if($image && ($content=='textimage' || $content=='titleimage' || $content=='image')) {
	if($shadow) $shadowsize='shadow="'.$shadow.'"'; else $shadowsize='';
	$image='<a href="'. get_permalink() .'" title="'.get_the_title().'" class="'.$image_align.'">[imageeffect type="'.$image_effect.'" align="'.$image_align.'" '.$shadowsize.' width="'.$image_width.'" height="'.$image_height.'"  alt="'. get_the_title().'" url="'.$image.'"  ]</a>';
	} else {
	$image='';	
	} ?>
		...
		the_advanced_excerpt('length='.$recent_excerpt); ?> 
        
        <a class="read-more" href="<?php the_permalink(); ?>"><?php _e( 'Lire plus &rarr;', 'DynamiX' ); ?></a>
        <?php } ?>
        </div>
        <?php } ?>
		<?php if($metadata=='yes') { ?>
        <div class="recent-metadata">
        <?php echo __('by', 'NorthVantage' ); ?> <a href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>"><?php echo get_the_author_firstname() ." ". get_the_author_lastname(); ?></a> <span class="subbreak">/</span> 
