...
if($DYN_gallerycat || $DYN_productcat || $DYN_producttag) {
	...
	if($DYN_productcat)
	foreach ($DYN_productcat as $pcatlist) { // Get category ID Array
		$pcats = $pcats.",".$pcatlist;
	}	

	if($DYN_producttag)
	foreach ($DYN_producttag as $taglist) { // Get category ID Array
		$tags = $tags.",".$taglist;
	}	
	...
	if(!$DYN_gridshowposts && !$DYN_gridfilter) { // Show number of posts on a page
		$DYN_gridshowposts="6";
	} elseif($DYN_gridfilter && $DYN_gallerynumposts) {
		$DYN_gridshowposts=$DYN_gallerynumposts;
...
elseif($DYN_productcat || $DYN_producttag) :
	...
	if($wpsc_product_categories) {
		foreach ($wpsc_product_categories as $wpsc_product_category) {
			$categories .= str_replace(" ","_", $wpsc_product_category->name). ' ';
		}
	}
	...
	if($wpsc_product_tags) {
		foreach ($wpsc_product_tags as $wpsc_product_tag) {
			$categories .= str_replace(" ","_", $wpsc_product_tag->name). ' ';
		}
	}	
	$categories = preg_replace("/&#?[a-z0-9]+;/i","",$categories);
endif;
